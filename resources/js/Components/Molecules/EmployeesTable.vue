<template>
  ...
  >
    ...
    <template #cell-job_title="{ row }">
      <div class="text-sm text-gray-900">{{ row.job_title }}</div>
      <div class="text-sm text-gray-500">{{ row.department }}</div>
    </template>

    <template #cell-hire_date="{ row }">
      <div class="text-sm text-gray-900">{{ formatDate(row.hire_date) }}</div>
    </template>
...
const columns = [
  ...
  { key: 'job_title', label: 'Position', sortable: true, searchable: true },
