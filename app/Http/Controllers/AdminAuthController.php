...
class AdminAuthController extends Controller
{
    ...
    public function logout(Request $request)
    {
        ...
        if (!$user) {
            return redirect('/admin/login')->with('error', 'No active session found.');
        }
        ...
        try {
            ...
            $this->logUserLogout($user, $request, 'logout_admin_user');
            
            // Perform logout operations
            $this->performLogout($request);
            
            // Return success response for Inertia.js
            return redirect('/admin/login')->with('message', 'Admin logout successful.');
            
        } catch (\Exception $e) {
            // Log error and still perform logout
            $this->logLogoutError($e, $user, $request, 'Admin logout error');
            $this->performLogout($request);
            
            return redirect('/admin/login')->with('error', 'Logout completed with some issues.');
        }
    }
